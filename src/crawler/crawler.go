...
func New(task *Task, retCh chan<- MTS) *Crawler {
	return &Crawler{
		task:  task,
		retCh: retCh,
	}
...
func (s *Crawler) SetDB(db *sql.DB) {
	s.db = db
}

// Run run a Crawler task
func (s *Crawler) Run() error {
	...
	for i := 0; i < nodesLen; i++ {
		...
		if s.task.OutputConfig.Type == OutputTypeMySQL {
			if s.db != nil {
	...
	go func() {
		...
		s.retCh <- MTS{ID: s.task.ID, Status: TaskStatusCompleted}
